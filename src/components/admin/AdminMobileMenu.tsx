...
import {
  Menu, X, TrendingUp, LayoutDashboard, Building2, Users, FileText,
  MessageSquare, Megaphone, LogOut, ClipboardList, ClipboardCheck,
  ListChecks, BarChart3, UserPlus, Kanban, FileCheck, ScrollText,
  CalendarCheck, DollarSign, Layers, RefreshCw, Receipt, AlertCircle, BarChart2,
} from "lucide-react";
...
function NavSection({ items, pathname, onClose }: { items: NavItem[]; pathname: string; onClose: () => void }) {
  return (
    <>
  ...
  return (
    <>
      <button
        ...
        className="lg:hidden p-2 rounded-lg text-[#0F3D5E] hover:bg-slate-100 transition-colors"
        ...
        {open && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 bottom-0 w-72 bg-[#0F3D5E] z-50 lg:hidden flex flex-col"
            >
              ...
              <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full">
                <NavSection items={mainItems} pathname={pathname} onClose={close} />

                <div className="pt-3 pb-1">
                  <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Financeiro</p>
                </div>
                <NavSection items={financeItems} pathname={pathname} onClose={close} />

                <div className="pt-3 pb-1">
                  <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Faturamento</p>
                </div>
                <NavSection items={billingItems} pathname={pathname} onClose={close} />

                <div className="pt-3 pb-1">
                  <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">Workflow</p>
                </div>
                <NavSection items={workflowItems} pathname={pathname} onClose={close} />

                <div className="pt-3 pb-1">
                  <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-white/30">CRM</p>
                </div>
                <NavSection items={crmItems} pathname={pathname} onClose={close} />
              </nav>
              ...
              <div className="px-3 pb-4 border-t border-white/10 pt-3">
                <Link
                  ...
                  onClick={close}
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium text-white/40 hover:text-white/60 transition-colors mb-1"
                >
                  Portal do cliente →
                </Link>
                <form action={signOut}>
